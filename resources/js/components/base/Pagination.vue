<template>
  <nav class="pagination is-centered" role="navigation" aria-label="pagination">
    ...
    <ul class="pagination-list">
      ...
      <li v-if="current > 1">
        <a class="pagination-link" :aria-label="['Goto page', prevP]" @click="previousPage">{{
          prevP
        }}</a>
      </li>
      <li>
      ...
      <li v-if="current < last - 1">
        <a class="pagination-link" :aria-label="['Goto page', nextP]" @click="nextPage">{{
          nextP
        }}</a>
      </li>
      <li v-if="current < last - 2"><span class="pagination-ellipsis">&hellip;</span></li>
      <li v-if="current != last">
        <a class="pagination-link" :aria-label="['Goto page', last]" @click="lastPage">{{
          lastP
        }}</a>
      </li>
    </ul>
