<template>
  <tr @mouseenter="masuk" @mouseleave="keluar" :class="selected">
    ...
    <td>
      ...
      <tr class="inner" :class="selected">
        <td class="dalam-tabel"><i class="fas fa-envelope"></i></td>
        <td class="dalam-tabel">{{ data.email }}</td>
      </tr>
      <tr class="inner" :class="selected">
      ...
      <p><i class="fas fa-id-card"></i> {{ data.email }}</p>
...
export default {
  ...
  computed: {
    ...
    status() {
      return this.data.status == 0 ? 'Belum Aktif' : this.data.status == 1 ? 'Aktif' : 'data tidak ditemukan';
    },
    icon() {
      return this.data.status == 2 ? 'fa-check' : 'fa-times-circle';
    },
    statusClass() {
      return this.data.status == 0 ? 'yellow' : this.data.status == 1 ? 'warna-tema' : 'yellow';
    },
    warna() {
